...
var (
	...
	rootCmd = &cobra.Command{
		Use:   "gateway",
		Short: "The Storx client-side S3 gateway",
...
func init() {
	defaultConfDir := fpath.ApplicationDir("storx", "gateway")
...
func cmdRun(cmd *cobra.Command, args []string) (err error) {
	...
	zap.S().Infof("Starting Storx DCS S3 Gateway\n\n")
...
func (flags GatewayFlags) Run(ctx context.Context) (err error) {
	...
	minio.Main([]string{"storx", "gateway", "storx",
...
func (flags GatewayFlags) NewGateway(ctx context.Context) (gw minio.Gateway, err error) {
	return miniogw.NewStorxGateway(flags.S3), nil
}
...
* See https://docs.storx.io/api-reference/s3-gateway for some example commands`)
