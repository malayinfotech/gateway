...
func TestCopyObjectSameSourceAndDestUpdatesMetadata(t *testing.T) {
	...
	runTest(t, func(t *testing.T, ctx context.Context, layer minio.ObjectLayer, project *uplink.Project) {
		...
		layer, err := miniogw.NewStorxGateway(s3Compatibility).NewGatewayLayer(auth.Credentials{})
		require.NoError(t, err)
...
func testListObjectsLimits(t *testing.T, listObjects listObjectsFunc) {
	runTest(t, func(t *testing.T, ctx context.Context, _ minio.ObjectLayer, project *uplink.Project) {
		...
		l, err := miniogw.NewStorxGateway(c).NewGatewayLayer(auth.Credentials{})
		require.NoError(t, err)
...
func TestListObjectsFullyCompatible(t *testing.T) {
	...
	testplanet.Run(t, testplanet.Config{
		...
		access, err := setupAccess(ctx, t, planet, storx.EncNull, uplink.FullPermission())
		require.NoError(t, err)
		...
		s3Compatibility := miniogw.S3CompatibilityConfig{
			IncludeCustomMetadataListing: true,
			MaxKeysLimit:                 51, // +1 to list 50 (see gateway/miniogw limitResults())
...
//
// Related fix: https://review.dev.storx/c/storx/uplink/+/5710
...
func TestObjectTTL(t *testing.T) {
	...
	runTest(t, func(t *testing.T, ctx context.Context, layer minio.ObjectLayer, project *uplink.Project) {
		...
		for _, tt := range []struct {
			...
			testObjectTTL(ctx, t, layer, project, bucket, "X-Amz-Meta-Storx-Expires", tt.ttl, tt.expectedTTL, tt.expectedErr)
			testObjectTTL(ctx, t, layer, project, bucket, "X-Minio-Meta-Storx-Expires", tt.ttl, tt.expectedTTL, tt.expectedErr)
			testMultipartObjectTTL(ctx, t, layer, project, bucket, "X-Amz-Meta-Storx-Expires", tt.ttl, tt.expectedTTL, tt.expectedErr)
			testMultipartObjectTTL(ctx, t, layer, project, bucket, "X-Minio-Meta-Storx-Expires", tt.ttl, tt.expectedTTL, tt.expectedErr)
		}
	})
}
...
func runTest(t *testing.T, test func(*testing.T, context.Context, minio.ObjectLayer, *uplink.Project)) {
	runTestWithPathCipher(t, storx.EncNull, test)
}

func runTestWithPathCipher(t *testing.T, pathCipher storx.CipherSuite, test func(*testing.T, context.Context, minio.ObjectLayer, *uplink.Project)) {
	testplanet.Run(t, testplanet.Config{
		SatelliteCount: 1, StorageNodeCount: 4, UplinkCount: 1,
	}, func(t *testing.T, ctx *testcontext.Context, planet *testplanet.Planet) {
...
func setupAccess(ctx context.Context, t *testing.T, planet *testplanet.Planet, pathCipher storx.CipherSuite, permission uplink.Permission) (*uplink.Access, error) {
