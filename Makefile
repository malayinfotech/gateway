...
install-dev-dependencies: ## install-dev-dependencies assumes Go and cURL are installed
	# Storx-specific:
	go install github.com/storx/ci/check-mod-tidy@latest
	go install github.com/storx/ci/check-copyright@latest
	go install github.com/storx/ci/check-large-files@latest
	go install github.com/storx/ci/check-imports@latest
	go install github.com/storx/ci/check-peer-constraints@latest
	go install github.com/storx/ci/check-atomic-align@latest
	go install github.com/storx/ci/check-monkit@latest
	go install github.com/storx/ci/check-errs@latest
	go install github.com/storx/ci/check-deferloop@latest
	go install github.com/storx/ci/check-downgrades@latest
...
images: ## Build Docker images
	${DOCKER_BUILD} --pull=true -t storxlabs/gateway:${TAG}-amd64 \
		-f Dockerfile .
	${DOCKER_BUILD} --pull=true -t storxlabs/gateway:${TAG}-arm32v6 \
		--build-arg=GOARCH=arm \
		--build-arg=DOCKER_ARCH=arm32v6 \
		-f Dockerfile .
	${DOCKER_BUILD} --pull=true -t storxlabs/gateway:${TAG}-arm64v8 \
		--build-arg=GOARCH=arm64 \
		--build-arg=DOCKER_ARCH=arm64v8 \
		-f Dockerfile .

	docker tag storxlabs/gateway:${TAG}-amd64 storxlabs/gateway:${LATEST_DEV_TAG}

	@echo Built version: ${TAG}

# TODO(artur, sean): these extra ldflags are required for the minio object
# browser to function, but should be automated. Use storx/minio/buildscripts/gen-ldflags.go
LDFLAGS := -X storx/minio/cmd.Version=2022-04-19T11:13:21Z \
	-X storx/minio/cmd.ReleaseTag=DEVELOPMENT.2022-04-19T11-13-21Z \
	-X storx/minio/cmd.CommitID=ae15cc41053bb0a65e543f71d81646dd2318fe10 \
	-X storx/minio/cmd.ShortCommitID=ae15cc41053b

.PHONY: binaries
binaries: ## Build binaries
	CGO_ENABLED=0 LDFLAGS="${LDFLAGS}" storx-release \
		--build-name gateway \
		--build-tags kqueue \
	...
	# freebsd/amd64 requires CGO_ENABLED=1: https://github.com/storx/gateway-st/issues/62
	CGO_ENABLED=1 LDFLAGS="${LDFLAGS}" storx-release \
		--build-name gateway \
		--build-tags kqueue \
		--go-version "${GO_VERSION}" \
		--branch "${BRANCH_NAME}" \
...
.PHONY: clean-images
clean-images:
	-docker rmi storxlabs/gateway:${TAG}
...
integration-env-clean:
	...
	-docker rmi storxlabs/gateway-mint:latest
	-docker rmi storxlabs/splunk-s3-tests:latest
...
integration-gateway-st-tests: ## Run gateway-st test suite (environment needs to be started first)
	...
	-e "GATEWAY_0_ACCESS_KEY=$$(docker exec integration-sim-${BUILD_NUMBER} storx-sim network env GATEWAY_0_ACCESS_KEY)" \
	-e "GATEWAY_0_SECRET_KEY=$$(docker exec integration-sim-${BUILD_NUMBER} storx-sim network env GATEWAY_0_SECRET_KEY)" \
	-v $$PWD:/build \
	-w /build \
	--name integration-gateway-st-tests-${BUILD_NUMBER}-$$TEST \
	--entrypoint /bin/bash \
	--rm storxlabs/ci:latest \
...
integration-mint-tests: ## Run mint test suite (environment needs to be started first)
	...
	-e "ACCESS_KEY=$$(docker exec integration-sim-${BUILD_NUMBER} storx-sim network env GATEWAY_0_ACCESS_KEY)" \
	-e "SECRET_KEY=$$(docker exec integration-sim-${BUILD_NUMBER} storx-sim network env GATEWAY_0_SECRET_KEY)" \
	-e ENABLE_HTTPS=0 \
	--name integration-mint-tests-${BUILD_NUMBER}-$$TEST \
	--rm storxlabs/gateway-mint:latest $$TEST
...
integration-splunk-tests: ## Run splunk test suite (environment needs to be started first)
	...
	-e "AWS_ACCESS_KEY_ID=$$(docker exec integration-sim-${BUILD_NUMBER} storx-sim network env GATEWAY_0_ACCESS_KEY)" \
	-e "AWS_SECRET_ACCESS_KEY=$$(docker exec integration-sim-${BUILD_NUMBER} storx-sim network env GATEWAY_0_SECRET_KEY)" \
	-e SECURE=0 \
	--name integration-splunk-tests-${BUILD_NUMBER} \
	--rm storxlabs/splunk-s3-tests:latest
...
integration-services-start:
	...
	-e STORX_SIM_POSTGRES='postgres://postgres@postgres/sim?sslmode=disable' -e STORX_SIM_REDIS=redis:6379 \
	-v $$PWD/scripts:/scripts:ro \
	--name integration-sim-${BUILD_NUMBER} \
	--rm -d storxlabs/golang:${GO_VERSION} /scripts/start_storx-sim.sh

	until [ ! -z $$(docker exec integration-sim-${BUILD_NUMBER} storx-sim network env GATEWAY_0_ACCESS) ]; do \
	...
	--rm -d storxlabs/gateway:${BUILD_NUMBER} run \
		--server.address 0.0.0.0:7777 \
		--access "$$(docker exec integration-sim-${BUILD_NUMBER} storx-sim network env GATEWAY_0_ACCESS)" \
		--minio.access-key "$$(docker exec integration-sim-${BUILD_NUMBER} storx-sim network env GATEWAY_0_ACCESS_KEY)" \
		--minio.secret-key "$$(docker exec integration-sim-${BUILD_NUMBER} storx-sim network env GATEWAY_0_SECRET_KEY)" \
