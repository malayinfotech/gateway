...
| GetBucketVersioning                         | No      | Planned. See https://github.com/storx/roadmap/issues/23      |                                                     |
...
| ListBuckets                                 | Full    |                                                              |                                                     |
| ListMultipartUploads                        | Partial | Planned full. See https://github.com/storx/roadmap/issues/20 | See ListMultipartUploads section                    |
| ListObjectVersions                          | No      | Planned. See https://github.com/storx/roadmap/issues/23      |                                                     |
| ListObjects                                 | Partial | Planned full. See https://github.com/storx/roadmap/issues/20 | See ListObjects section                             |
| ListObjectsV2                               | Partial | Planned full. See https://github.com/storx/roadmap/issues/20 | See ListObjects section                             |
| ListParts                                   | Full    |                                                              |                                                     |
...
| PutBucketVersioning                         | No      | Planned. See https://github.com/storx/roadmap/issues/23      |                                                     |
...
order. Forcing exhaustive listing for any request is not possible for Storx
...
[storx/gateway-mt#213](https://github.com/storx/gateway-mt/issues/213)
...
[storx/gateway-mt#149](https://github.com/storx/gateway-mt/issues/149).
...
improve object consistency in [storx/storx#4788](https://github.com/storx/storx/issues/4788).

### ACL-related actions

[Secure access control in the decentralized
cloud](https://www.storx/blog/secure-access-control-in-the-decentralized-cloud)
...
Storx don't impose such a limit, but the existence of the limit in AWS S3
...
```console
$ aws --profile storx configure set s3.multipart_chunksize 630MiB
$ aws --profile storx --endpoint-url https://gateway.storxshare.io s3 cp 6TiB_file s3://objects/
```

## Storx-specific extensions

### Object-level TTL

It's possible to specify TTL for the object by sending the

- `X-Amz-Meta-Storx-Expires` or
- `X-Minio-Meta-Storx-Expires`
...
```console
$ aws s3 --endpoint-url https://gateway.storxshare.io cp file s3://bucket/object --metadata Storx-Expires=+2h
upload: ./file to s3://bucket/object

# or

$ aws s3 --endpoint-url https://gateway.storxshare.io cp file s3://bucket/object --metadata Storx-Expires=2022-05-19T00:10:55Z
upload: ./file to s3://bucket/object
```

#### Caveats

The value under `X-Amz-Meta-Storx-Expires` has priority over the value under
`X-Minio-Meta-Storx-Expires`.
...
func main() {
	...
	ses := session.Must(session.NewSession(aws.NewConfig().WithCredentials(creds).WithRegion("eu1").WithEndpoint("https://gateway.storxshare.io")))
	svc := s3.New(ses)
